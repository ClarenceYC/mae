...
def noisify_pairflip(y_train, noise, random_state=None, nb_classes=10):
    ...
    P = np.eye(nb_classes)
    n = noise
    # noise_arr = [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45]
    # count = 0
    ...
    if n > 0.0:
        ...
        # temp_n = noise_arr[count]
        # count = (count + 1) % len(noise_arr)
        # P[0, 0], P[0, 1] = 1. - temp_n, temp_n
        # for i in range(1, nb_classes-1):
        #     temp_n = noise_arr[count]
        #     count = (count + 1) % len(noise_arr)
        #     P[i, i], P[i, i + 1] = 1. - temp_n, temp_n
        # temp_n = noise_arr[count]
        # count = (count + 1) % len(noise_arr)
        # P[nb_classes-1, nb_classes-1], P[nb_classes-1, 0] = 1. - temp_n, temp_n
...
def noisify_multiclass_symmetric(y_train, noise, random_state=None, nb_classes=10):
    ...
    #     P[i, i] = noise_arr[count]
    #     count = (count + 1) % len(noise_arr)
    #     for j in range(0, nb_classes):
    #         if j != i:
    #             P[i, j] = (1 - P[i, i]) / (nb_classes - 1)
